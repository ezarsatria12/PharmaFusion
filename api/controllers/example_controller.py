...
def predict_example():
    try:
        ...
        # Query data dari database berdasarkan Disease_ID
        description_data = DescriptionModel.query.filter_by(Symptomps_ID=int(disease_id)).first()

        if description_data:
            # Kombinasikan hasil prediksi dengan data dari database
            result = {
                "Disease_ID": disease_id,
                "penyakit": disease,
                "probabilitas": float(probability),  # Pastikan tipe data probabilitas adalah float
                "description": description_data.to_dict()  # Gunakan fungsi to_dict() dari model
            }
        else:
            result = {
                "Disease_ID": disease_id,
